...
import update_package.algorithms.apply_hunting as apply_hunting
import update_package.algorithms.apply_time_offset as apply_time_offset
...
def apply_fault_label_statistics(data,count):
    ...
    tro_model_relative_path = os.path.join(current_dir,".." , "model","tro_group_model")
    # 상대 경로를 절대 경로로 변환
    ...
    # 데이터 적재
    load_database(group,'tc_ai_fault_model_group')
...
def model_predict(data):
    ...
    tro_model_path = os.path.join(parent_dir,'model/tro_model')
...
def apply_fault_algorithms(data):
    """ 인자: 선박 이름, 오퍼레이션 번호, 섹션 번호
        ...
        반환 값: 오퍼레이션 실시간 데이터 자동적재, 오퍼레이션 그룹 자동적재
    """
    ...
    # 4. 자동 적재
    # load_database(data,'tc_ai_fault_label')
