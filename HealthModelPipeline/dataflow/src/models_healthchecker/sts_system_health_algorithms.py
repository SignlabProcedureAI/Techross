...
def catorize_health_score(data):
    ...
    data.loc[data['HEALTH_SCORE']<=9, 'RISK'] = 'NORMAL'
    data.loc[(data['HEALTH_SCORE']>9) & (data['HEALTH_SCORE']<=20), 'RISK'] = 'WARNING'
    data.loc[(data['HEALTH_SCORE']>20) & (data['HEALTH_SCORE']<=50), 'RISK'] = 'RISK'
    data.loc[data['HEALTH_SCORE']>50, 'RISK'] = 'DEFECT'
...
def apply_system_health_statistics_with_sts(data):
    ...
    data = data[['SHIP_ID','OP_INDEX','DATA_INDEX','SECTION','CSU','FTS','FMU','CURRENT','TRO','STS','DIFF','THRESHOLD',
           'HEALTH_RATIO','HEALTH_TREND']]


    # 데이터 그룹화
    group = data.groupby(['SHIP_ID','OP_INDEX','SECTION']).agg({'DATA_INDEX':'mean','CSU':'mean','FTS':'mean','FMU':'mean','CURRENT':'mean','TRO':'mean','STS':['min','mean','max'],'DIFF':['min','mean','max'],'THRESHOLD':'mean','HEALTH_RATIO':'mean','HEALTH_TREND':'mean'})
    
    # 다중 인덱스된 컬럼을 단일 레벨로 평탄화
    group.columns = ['_'.join(col) for col in group.columns]
    group.columns = ['DATA_INDEX','CSU','FTS','FMU','CURRENT','TRO','STS_MIN','STS_MEAN','STS_MAX','DIFF_MIN','DIFF_MEAN','DIFF_MAX','THRESHOLD','HEALTH_RATIO','HEALTH_TREND']

    # SCORE 변수 추가
    score, trend_score = calculate_group_health_score(data,'STS')
    ...
    group = group[['SHIP_ID','OP_INDEX','SECTION','OP_TYPE','CSU','FTS','FMU','CURRENT','TRO','STS_MIN','STS_MEAN','STS_MAX','DIFF_MIN','DIFF_MEAN','DIFF_MAX','THRESHOLD','TREND_SCORE',
           'HEALTH_RATIO','HEALTH_TREND','HEALTH_SCORE','START_TIME','END_TIME','RUNNING_TIME']]
    
    # 모델 로드

    # 현재 파일의 경로를 기준으로 model 폴더 내 sts_model 경로 생성
    current_dir = os.path.dirname(os.path.abspath(__file__))
    sts_model_relative_path = os.path.join(current_dir,".." , "models_model","sts_model")
    # 상대 경로를 절대 경로로 변환
    sts_model_path = os.path.abspath(sts_model_relative_path)

    model = load_model_from_pickle(sts_model_path)
    
    # 변수 선택 및 예측
    X = group[['STS_MIN','STS_MEAN','STS_MAX','DIFF_MIN','DIFF_MEAN','DIFF_MAX','TREND_SCORE']]
    pred =  model.predict(X)

    group['PRED'] = pred

    # 학습 데이터 적재
    load_database('test','tc_ai_mclr_sts_system_health_group', group)
    ...
    # 뷰 데이터 적재
    load_database('test','tc_ai_sts_model_system_health_group', group)
...
def generate_health_score(data,col):
    ...
    pre.columns=['SHIP_ID','OP_INDEX','SECTION','OP_TYPE','DATA_TIME','DATA_INDEX','CSU','STS','FTS','FMU','CURRENT','TRO','RATE','VOLTAGE',
                 'START_TIME','END_TIME','RUNNING_TIME','STS_Ratio','THRESHOLD','HEALTH_RATIO','HEALTH_TREND']
...
def apply_system_health_algorithms_with_sts(data):
    ...
    # 3. 데이터 프레임 재 설정
    position_columns =  ['SHIP_ID','OP_INDEX','SECTION','OP_TYPE','DATA_TIME','DATA_INDEX','CSU','FTS','FMU','CURRENT','TRO','STS','DIFF',
