...
def plot_histograms_with_noise(df,idx,reg,str_op_type):
    """
    ...
    #if boa == 'before':
    plt.savefig(f"D:\\bwms\\pract\\noise\\{reg}_{idx}_{str_op_type}.png", dpi=300, bbox_inches='tight')
    #else:
...
def plot_bar_with_operation(original_data,idx,reg):
    ...
    plt.tight_layout()
    plt.savefig(f"D:\\bwms\\pract\\operation\\{reg}_{idx}.png", dpi=300, bbox_inches='tight')
    #plt.show()
    
# In[20]:


def plot_pie_with_duplication(original_data,idx,reg):
    ...
    plt.tight_layout()
    plt.savefig(f"D:\\bwms\\pract\\duplication\\{reg}_{idx}.png", dpi=300, bbox_inches='tight')
    #plt.show()
    
# In[40]:


def plot_double_bar_with_missing(original_data,idx,reg): 
    ...
    # 그래프 보여주기
    plt.tight_layout()
    plt.savefig(f"D:\\bwms\\pract\\missing\\{reg}_{idx}.png", dpi=300, bbox_inches='tight')
    #plt.show()
