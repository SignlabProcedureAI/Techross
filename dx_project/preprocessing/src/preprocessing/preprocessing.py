...
def apply_preprocessing_fuction(ship_id, op_index, section, data):
    ...
    # 설명 텍스트 생성
    text_dict = generate_supplementary_explanation_text(duplication_text, missing_values_text, operation_values_text, text_list, indicator_df)

    # 처리 개수 데이터 적재
    generate_preprocessing_count_dataframe(ship_id, op_index, section, duplication_len, missing_len, operation_len, noise_len)
    
    return sensor, organized_data, indicator_df, text_dict
...
def generate_supplementary_explanation_text(duplication_text, missing_values_text, operation_values_text, text_list, indicator_df):
    ...
    noise = indicator_df['NOISE']
    missing = indicator_df['MISSING']
    duplication = indicator_df['DUPLICATE']
    operation = indicator_df['OPERATION']
...
def process_preprocessed_list(indicator_data, source_len, pre_len): 
    ...
    # 딕셔너리 생성하다
    indicator_dict = {'SHIP_ID':None, 'OP_INDEX':None, 'SECTION':None,  'OP_TYPE':None,  'NOISE':None, 'MISSING':None, 'DUPLICATE':None, 'OPERATION':None, 'DATA_COUNT':None, 'PRE_COUNT':None, 'START_DATE':None, 'END_DATE':None}
...
def generate_preprocessing_count_dataframe(ship_id, op_index, section, duplication_len, missing_len, operation_len, noise_len):
    
    count_dict = [{'SHIP_ID':ship_id, 'OP_INDEX':op_index, 'SECTION':section, 'NOISE':noise_len ,'MISSING':missing_len, 'DUPLICATE':duplication_len, 'OPERATION':operation_len}]
